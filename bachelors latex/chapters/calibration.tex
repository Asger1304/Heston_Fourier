...
\begin{itemize}
  ...
  \item too wide spread: drop if $(\text{ask}_i-\text{bid}_i)/\text{mid}_i>0.5$
\end{itemize}
...
\begin{figure}[H]
  ...
  \caption{Calibration speed experiment: wait time and $\sum|\Delta\theta|$ relative to the GK baseline across 20 random Heston parameter draws satisfying the Feller condition. Code for replication can be found in the repo.}
...
Some of this improvement will most certainly be due to a better implementation of one or the other; cutting the calibration to a tenth is still quite nice I think. Well, anyway, for this project
speed is a non-issue, meaning we might as well take the slow approach that we know works. Going forward we'll simply use the GK approach with the numerical gradient.
