...
instead of writing this every time we will simply define this as the default probability measure. Usually we like to note information up to a time as $\mathcal{F}_t$ in this paper I've chosen to drop the filters i.e work from the filter $\mathcal{F}_0$. One should 
...
$\mathbb{E}_\mu[f(X_{t+s},V_{t+s})|\mathcal{F}]=\mathbb{E}_{(x_t,v_t)}[f(X_s,V_s)]$ for $f$ non negative and measurable, so this actually doesn't change anything simply set $T=T-t\quad x_0=x_t,v_0=v_t$

For ease of replication of the results in this paper i've created a Notebook running all the code, gennerating graphs/results. This along with all of the code is available here\cite{github_repo}.
